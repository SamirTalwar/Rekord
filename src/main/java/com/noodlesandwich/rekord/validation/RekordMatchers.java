...
public final class RekordMatchers {
    ...
    public static <T> Matcher<Rekord<T>> that(final Check<T> check) {
        return new TypeSafeDiagnosingMatcher<Rekord<T>>() {
            @Override
    ...
    public static <T, V> Matcher<Rekord<? extends T>> hasKey(final Key<T, V> key) {
        return new TypeSafeDiagnosingMatcher<Rekord<? extends T>>() {
            @Override
            public void describeTo(Description description) {
            ...
            @Override
            protected boolean matchesSafely(Rekord<? extends T> rekord, Description mismatchDescription) {
    ...
    public static <T> Matcher<Rekord<T>> allProperties() {
        return new TypeSafeDiagnosingMatcher<Rekord<T>>() {
            @Override
            public void describeTo(Description description) {
    ...
    @SafeVarargs
    public static <T> Matcher<Rekord<T>> hasProperties(final Key<? super T, ?>... keys) {
    ...
    public static <T, V> Matcher<Rekord<T>> hasProperty(final Key<T, V> key, final Matcher<V> expectedValue) {
        return new TypeSafeDiagnosingMatcher<Rekord<T>>() {
            @Override
            public void describeTo(Description description) {
                description
